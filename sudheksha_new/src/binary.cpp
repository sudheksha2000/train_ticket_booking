...
binary_tree_class::binary_tree_class()
{
	...
	binary_tree->A_M= new age_ptr(1);
	binary_tree->N_Z=new age_ptr(1);
...
void binary_tree_class :: binary_search (user_derived * ptr)
{
...
user_derived * binary_tree_class:: check(long int id)
{
	...
	while(current)
	{
		...
		current=current->next;            //user_list
	}
	return ptr;
}
